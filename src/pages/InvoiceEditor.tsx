...
const invoiceLineSchema = Yup.object().shape({
  ...
  itemID: Yup.number().min(1, "Please select an item").required("Item is required"),
  ...
  discountPct: Yup.number().min(0, "Discount must be 0 or greater").max(100, "Discount must be 100 or less"),
});
...
const invoiceSchema = Yup.object().shape({
  ...
  taxPercentage: Yup.number().min(0, "Tax must be 0 or greater").max(100, "Tax must be 100 or less"),
  notes: Yup.string().trim().max(1000, "Notes must be less than 1000 characters"),
  lines: Yup.array().of(invoiceLineSchema).min(1, "At least one line item is required"),
});
...
const InvoiceEditor = () => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      notes: "",
      lines: [
        { rowNo: 1, itemID: 0, description: "", quantity: 1, rate: 0, discountPct: 0 }
      ] as InvoiceLine[],
    },
  ...
  const loadInvoice = async () => {
    ...
    try {
      ...
      formik.setValues({
        ...
        notes: data.notes || "",
        lines: data.lines && data.lines.length > 0 ? data.lines : [
          { rowNo: 1, itemID: 0, description: "", quantity: 1, rate: 0, discountPct: 0 }
        ],
      });
    } catch (error) {
  ...
  const calculateLineAmount = (line: InvoiceLine) => {
    ...
    const discount = subtotal * (line.discountPct / 100);
  ...
  const addLine = () => {
    ...
    const newRowNo = lines.length > 0 ? Math.max(...lines.map(l => l.rowNo)) + 1 : 1;
    formik.setFieldValue('lines', [...lines, { rowNo: newRowNo, itemID: 0, description: "", quantity: 1, rate: 0, discountPct: 0 }]);
  };
  ...
  const removeLine = (index: number) => {
    ...
    formik.setFieldValue('lines', formik.values.lines.filter((_, i) => i !== index));
  };

  const subTotal = formik.values.lines.reduce((sum, line) => sum + calculateLineAmount(line), 0);
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                {formik.touched.invoiceNo && formik.errors.invoiceNo && (
                  <p className="text-sm text-destructive">{formik.errors.invoiceNo}</p>
                )}
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                {formik.touched.taxPercentage && formik.errors.taxPercentage && (
                  <p className="text-sm text-destructive">{formik.errors.taxPercentage}</p>
                )}
              </div>
            </CardContent>
          </Card>
                    ...
                    <TableRow key={index}>
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) => handleLineChange(index, 'itemID', parseInt(value))}
                        >
                          <SelectTrigger>
                        ...
                        {formik.touched.lines?.[index]?.itemID && formik.errors.lines?.[index] && typeof formik.errors.lines[index] === 'object' && (formik.errors.lines[index] as any).itemID && (
                          <p className="text-sm text-destructive mt-1">{(formik.errors.lines[index] as any).itemID}</p>
                        )}
                      </TableCell>
                      <TableCell>
                        <Input
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => handleLineChange(index, 'discountPct', parseFloat(e.target.value) || 0)}
                          className="text-right"
                        />
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        {company?.currencySymbol}{calculateLineAmount(line).toFixed(2)}
                      </TableCell>
                      <TableCell>
                        <Button
                          type="button"
            ...
            <div className="mt-6 flex justify-end">
              <div className="w-80 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{company?.currencySymbol}{subTotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Tax ({formik.values.taxPercentage}%):</span>
                  <span className="font-medium">{company?.currencySymbol}{taxAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-lg font-bold border-t pt-2">
                  <span>Total:</span>
                  <span>{company?.currencySymbol}{total.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="shadow-card">
          ...
          <CardContent>
            ...
            {formik.touched.notes && formik.errors.notes && (
              <p className="text-sm text-destructive mt-2">{formik.errors.notes}</p>
            )}
          </CardContent>
        </Card>
      </form>
